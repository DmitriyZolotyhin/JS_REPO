...
function onKeyDownProcess(input, listItem, savedValue, event) {
    if(event.key === 'Enter')
        editTask(listItem);
...
function editTask(listItem) {
    ...
    if (containsClass) {
        ...
        input.onkeydown = onKeyDownProcess.bind(null, input, listItem, val);
    }
...
function search(str) {
    ...
    const variable = textElement.includes(text) || text === '';
...
function search() {
    ...
    const word = searchLine.value;
    loadData(word);
}

function loadData(searchElement=undefined) {
    const data = load();
    unfinishedTasks.innerHTML = '';
    finishedTasks.innerHTML = '';
    for (let i = 0; i < data.unfinishedTasks.length; i++) {
        const task = data.unfinishedTasks[i];
           
        const listItem = createNewElement(task , false);
        if(searchElement)
            listItem.children[1].innerHTML = listItem
                .children[1]
                .innerHTML
                .replaceAll(searchElement,`<b><span style="color:red"> ${searchElement} </span></b>`);
        unfinishedTasks.appendChild(listItem);
        bindTaskEvents(listItem , finishTask);
    }
    for (let i = 0; i < data.finishedTasks.length; i++) {
        const task = data.finishedTasks[i];
        const listItem = createNewElement(task , true);
        if(searchElement)
            listItem.children[1].innerHTML = listItem
                .children[1]
                .innerHTML
